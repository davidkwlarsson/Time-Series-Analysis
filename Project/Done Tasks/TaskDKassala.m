...
rain_K = getfield(Kassala,'rain'); %This is the interpolated data
rain_org_K = getfield(Kassala,'rain_org'); %actual rain data
rain_org_t_K = getfield(Kassala,'rain_org_t');
%% Divide into Model,Validation and test data:
nvdi_K = nvdi_K./255*2 - 1; % [0,255] ->[-1,1]
y = nvdi_K(1:450);
y_valid = nvdi_K(451:end);
y_test = nvdi_K(581:end);
...
acf(res,50,0.5,1);
MSE_Kassala = sum(res.^2)/length(res)
STD_Kassala = std(res)